...
namespace marray {
	...
	> struct tmultiarray: tindexeddata< T, PT, S, D > {
		...
		tmultiarray( const tmultiarray& rhs ) 
			: base_array( rhs ), _layout( rhs._layout ), _slice_ref() {}
		
		tmultiarray( 
			typename base_array::iterator begin, 
			const layout_type& layout 
		) : base_array( begin, layout.footprint() ), _layout( layout ), _slice_ref(){}
		
		const_reference
		operator()( const index_type& idx ) const {
			return base_array::operator[]( _layout.get_stride( idx ) );
		}
		...
		operator[]( size_type i) const {
			assert( i < dim(i) );
			this->_slice_ref.reset( this->begin() + i * _layout.slice(0).footprint(), _layout.slice(0) );
			return _slice_ref;
		}
		...
		operator[]( size_type i) {
			assert( i < dim(i) );
			cout << "foot 2: " << _layout.slice(0).footprint() <<endl;
		...
		*/
		size_t
		dim( size_t i ) const { return _layout.dim(i); }
		...
		*/
		void reset( iterator begin ) {
			this->reset( begin, begin + _layout.footprint() );
		}
		
		protected:
		
		layout_type _layout;
		mutable slice_type _slice_ref;
	};
	...
	> struct tmultiarray< T, 2, PT, S, D, W, L > : tindexeddata< T, PT, S, D > {
		...
		const_reference
		operator()( const index_type& idx ) const {
			return this->operator[]( _layout.get_stride( idx ) );
		}
		...
		operator[]( size_type i ) const {
			assert( i < dim(i) );
			_slice_ref.reset( this->begin() + i * _layout.slice(0).footprint(), this->begin() + i * _layout.slice(0).footprint() + _layout.dim(1) );
			return _slice_ref;
		}
		...
		operator[]( size_type i) {
			assert( i < dim(i) );
			cout << "foot 1: " << _layout.slice(0).footprint() <<endl;
			this->_slice_ref.reset( this->begin() + i * _layout.slice(0).footprint(), this->begin() + i * _layout.slice(0).footprint() + _layout.dim(1) );
			return _slice_ref;
		}
		...
		*/
		void reset( iterator begin ) {
			tindexeddata< T, PT, S, D >::reset( begin, begin + _layout.footprint() );
		}
	protected:
	...
	> struct tmultiarray< T, N, PT, S, D, false, L >  : tmultiarray< T, N, PT, S, D, true, L > {
		...
		*/
		size_t
		dim( size_t i ) const { return this->_layout.dim(i); }
